...
#include <MCP3XXX.h>
MCP3008 adc;
#define CS_PIN   24   //PA0  //PC15  //ss 8
#define MOSI_PIN  25  //PA1  //PC14  //di 7
#define MISO_PIN  26  //PA2  //PA15  //do 6
#define CLOCK_PIN 27  //PA3  //PB11  //clk 5
...
#define ALL 113
#define N 114
int Max[numsensor] = {735,  610,  538,  506,  610,  530,  591,  763};
int Min[numsensor] = { 33, 30, 30, 28, 30, 28, 29, 34};
...
void setup() {
  ...
  beep();
  glcdMode(3); //กลับเฟสตัวหนังสือ
  setTextSize(2); 
  glcd(2,0,"SW_OK --> RUN");
  glcd(3,0,"MARK Robotic");
...
void Read() {
  for (int i = 0; i < numsensor; i++) {
    ...
    int x = map(adc.analogRead(i), Min[i], Max[i], 1000, 0);
    if (x > 1000) x = 1000;
    if (x < 0) x = 0;
    s[i] = x;
  }
