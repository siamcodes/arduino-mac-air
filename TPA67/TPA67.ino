...
void setup() {
  ...
  while (!SW_OK()) {  //กดปุ่ม OK เพื่อเริ่มต้นทำงาน
    oled.text(2, 3, "L=%d ", analog(1));
    oled.text(2, 13, "R=%d ", analog(2));
    oled.text(3, 1, "VS1=%d ", val_S1);
    oled.text(3, 11, "VS2=%d ", val_S2);
    oled.text(4, 1, "VS0=%d ", val_S0);
    oled.text(4, 11, "VS3=%d ", val_S3);
    oled.text(5, 0, "TagL=%d ", analog(0));
    oled.text(5, 10, "TagR=%d ", analog(3));
    oled.show();
  }
...
void loop() {
      //อย่าลืม swa
      //if(status == 1) {/*วิ่งจากจุด Start*/ }
      if(status == 2) { Ball(); }
      if(status == 3) { popB(); }
      //if(status == 4) {/*วิงไปยังจุด End*/ }
