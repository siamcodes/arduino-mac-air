void mission1() {
  if (pixy.updateBlocks() && pixy.sigSize[1]) { //ลูกบอลสีส้ม s[1]
      ...
      //goalYellow();                              //หาประตู
        if (abs(rot_error) < rotErrorGap) {  //ถ้าลูกบอลอยู่ตรงกลางให้ทำการยิง
          ...
          while (1) {  //เดินหน้าตรงความเร็ว 100%
            getIMU();  //หรือลองใช้ updateIMU();
            heading(100, 90, 0);                      //เดินหน้า
          ...
          discoveState = 1;
          ///////////////////////
          while (1) {  //ยิงเสร็จถอยมาหยุดที่ฝั่งตนเอง
            getIMU();  //หรือลองใช้ updateIMU();
            heading(80, 270, 0);  //ุถอย
          ...
          getIMU();  //หรือลองใช้ updateIMU();    
          holonomic(70,90,0);  //เดินหน้านิดหนึ่ง
          delay(300);
