...
#define numsensor 8
int last_value = 0;
int MaxF[numsensor] = { 970,	975,	887,	916,	923,	850,	964,	680 };
int MinF[numsensor] = { 232,	251,	134,	131,	174,	143,	179,	123 };
int MaxB[numsensor] = { 957,	962,	861,	858,	965,	784,	858,	791 };
int MinB[numsensor] = { 188,	194,	150,	151,	212,	138,	166,	162 };
...
void setup() {
  ...
  //servo(2, openR);
  //servo(3, down);
  //un(2, 1, R, FRONTSENSOR, N);
  //tn(30,0.010, 0.10, FRONTSENSOR);
  //k(200);
  //tlr(20, 0.022, 0.22, FRONTSENSOR);
...
void Read(int Sensor) {
  ...
  if (Sensor == FRONTSENSOR) {
    ...
    adc.begin(A0, A2, A1, 8);
    for (int i = 0; i < numsensor; i++) {
      x = map(adc.readADC(i), MinB[i], MaxB[i], 800, 0); // 800,0
      if (x >= 800) x = 800;
      if (x <= 0) x = 0;
      B[i] = x;
    }
